...
using KernelAbstractions: Backend
using StaticArrays

# ── Batched GPU Tsit5 path: B electrons per kernel launch ──
#
# EXPERIMENTAL / under active development — not part of the promoted GPU API.
# Retained for ongoing electron-batching experiments; not yet validated against
# the reference solver (the committed accuracy test exercises GPUKernelRK4 only).
#
...
!!! warning "Experimental"
    This path is under active development and is **not** validated against the
    reference solver.  Use `GPUKernelRK4` for trusted results.
