...
launch_lane() {
    ...
    if lane_done "$i"; then
        ...
        # DIAG_CAMPAIGN (per-card campaign name of the diag_validate recipes) is forwarded: the pod
        # re-sources the recipe, so a driver-side override must travel with the launch.
