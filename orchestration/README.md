...
#   re-keys the depot warm cache). Unregistered deps (lib/RunManifests as a path dep; CountedFloats.jl
#   and GPUDiagnostics.jl as git-URL deps) need a [sources] entry in every env that path-devs the
#   package (scripts/, test/, benchmarks/, poster/, animation/), each listing them in its own
#   [deps] + [sources]; adding one means Pkg.resolve() + commit in each tracked manifest
