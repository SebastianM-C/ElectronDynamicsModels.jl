...
# merge the counters into the cell's run manifest as [gpu].hw_* keys. The GPM sampler does this
# in-process on NVIDIA; AMD has no in-process counter API, so the whole solver process is wrapped
# by the profiler (GPUDiagnostics.jl `hw_counter_command(RocprofV3(), …)`, under `timeout -k`
# because a counter set the hardware refuses aborts rocprofv3 with signal 6 and leaves the child
# hung) and the CSV it writes is parsed afterwards (`hw_counters` → scripts/hw_counter_merge.jl).
#
#   bash orchestration/profile_cell.sh <set|"COUNTER …"> <outdir> <tag> [EDM_VAR=val …]
#
#   <set>     an intent name from GPUDiagnostics.COUNTER_SETS[:amd] (issue, occupancy, memory,
#             fp64, l2; the 0.2 names sq_issue / sq_waves / l1_pipe are accepted as aliases) or a
#             space-separated list of rocprofv3 counter names (one pass: what the hardware can
#             collect at once — `rocprofv3 --list-avail` names them). Device-qualified names
#             (SQ_WAVES:device=0) pass through and are what rocprofv3 needs when it enumerates an
#             unsupported GPU beside the target one
...
( cd "$REPO" && "${JL[@]}" -t 1 --project=scripts -e '
    ...
    sel = haskey(COUNTER_SETS[:amd], sym) ? (; set = sym) : (; metrics = String.(split(spec)))
    cmd = hw_counter_command(RocprofV3(), Cmd(String.(ARGS[5:end])); dir, name, timeout_s, sel...)
...
grep -iE 'exceeds the capabilities|caught signal|unordered_map::at' "$out/rocprof_$tag.log" | head -3 >&2 || true
