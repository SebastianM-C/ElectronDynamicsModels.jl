...
the static instruction mix, the rocprofv3 counter path, and the `diagnostics_dict` report layer —
is the separate package [GPUDiagnostics.jl](https://github.com/SebastianM-C/GPUDiagnostics.jl),
...
`orchestration/profile_cell.sh` + `scripts/rocprof_merge.jl` run a solver cell under a rocprofv3
counter set and merge the result as `rocprof_*` keys. `scripts/instruction_mix.jl` prints the
production field kernel's instruction mix, natively or cross-compiled for a GPU that is not
present.
