...
@testset "write_solver_manifest ↔ run_spec_from_manifest contract" begin
    ...
    cfg2 = merge(config, Dict{String, Any}(
        ...
        "bunch_nb" => 398, "bunch_l" => -2, "positions" => "0.5,0;-0.5,0", "layout" => "pair_d1", "screen_z_lambda" => 1.0e7))
    ...
    @test env2["EDM_Z"] == "1.0e7"
    @test !haskey(spec.env, "EDM_TSPAN_TAU") && !haskey(spec.env, "EDM_WINDOW_LEAD") &&
          !haskey(spec.env, "EDM_BUNCH_NB") && !haskey(spec.env, "EDM_POSITIONS") && !haskey(spec.env, "EDM_Z")
...
@testset "screen_halfwidth / window_start — recorded, knob-derived, legacy" begin
    ...
    legacy_edge(hw) = c * base["setup"]["τi"] + hypot(base["setup"]["Z"], hw + base["setup"]["Rmax"])   # pre-fix EDGE anchor, reconstructed verbatim
    # Pre-knob manifest: the historical ±25 w₀ frame and its legacy (edge-anchored) window start.
    @test screen_halfwidth(base) ≈ 25w0
    @test window_start(base) ≈ legacy_edge(25w0)
    ...
    @test window_start(mt) ≈ legacy_edge(8w0)          # the grid AND the window follow the zoom
    ...
    @test window_start(mi) ≈ legacy_edge(4.5w0)   # |Z| in the legacy anchor
